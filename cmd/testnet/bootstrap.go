...
// this env variable is used in supported fury templates to allow override of the image tag
...
func BootstrapCmd() *cobra.Command {
	bootstrapCmd := &cobra.Command{
		Use:   "bootstrap",
		Short: "A convenience command that creates a fury testnet with the input configTemplate (defaults to master)",
		Long: `Generate a fury testnet and optionally run/integrate various other resources.
...
`,
		...
		RunE: func(_ *cobra.Command, _ []string) error {
			...
			// generate fury node configuration
			...
			// pull the fury image tag if not overridden to be "local"
	...
	bootstrapCmd.Flags().StringVar(&furyConfigTemplate, "fury.configTemplate", "master", "the directory name of the template used to generating the fury config")
	...
	bootstrapCmd.Flags().StringVar(&chainUpgradeName, "upgrade-name", "", "name of automated chain upgrade to run, if desired. the upgrade must be defined in the fury image container.")
	bootstrapCmd.Flags().Int64Var(&chainUpgradeHeight, "upgrade-height", 0, "height of automated chain upgrade to run.")
	bootstrapCmd.Flags().StringVar(&chainUpgradeBaseImageTag, "upgrade-base-image-tag", "", "the fury docker image tag that will be upgraded.\nthe chain is initialized from this tag and then upgraded to the new image.\nthe binary must be compatible with the fury.configTemplate genesis.json.")
...
func setupIbcChannelAndRelayer() error {
	...
	// open the channel between fury and ibcnode
...
func writeUpgradeProposal() (string, error) {
	...
	return "/root/.fury/config/upgrade-proposal.json", os.WriteFile(
		generatedPath("fury", "initstate", ".fury", "config", "upgrade-proposal.json"),
		[]byte(content),
		0644,
	)
}
...
// blockGTE is a backoff operation that uses fury's CLI to query the chain for the current block number
...
func blockGTE(n int64) backoff.Operation {
	return func() error {
		cmd := "fury status | jq -r .sync_info.latest_block_height"
...
// runFuryCli execs into the fury container and runs `fury args...`
