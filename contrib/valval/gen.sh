...
for ((i = 1; i <= num_validators; i++)); do
  ...
  if [ ! -d $home ]; then
    ...
    fury init val$i --home $home --chain-id furymirror_710-1 >/dev/null 2>&1

    rm -rf $home/data
    rm $home/config/genesis.json
    cp fury-1/config/init-data-directory.sh $home/config/init-data-directory.sh
    cp fury-1/config/priv_validator_state.json.example $home/config/priv_validator_state.json.example
  fi

  cp $home/config/priv_validator_key.json keys/priv_validator_key_$(($i - 1)).json
  peers+=("$(fury tendermint show-node-id --home $home)@$home:26656")
...
if [ "$there_is_a_new_validator" = true ]; then
  ...
  for ((i = 1; i <= num_validators; i++)); do
    configtoml=fury-$i/config/config.toml
    echo "$configtoml"
